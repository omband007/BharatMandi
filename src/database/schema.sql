...
CREATE INDEX idx_listings_produce_type ON listings(produce_type);
...
CREATE INDEX idx_escrow_is_locked ON escrow_accounts(is_locked);
...
CREATE INDEX idx_credibility_farmer_id ON credibility_scores(farmer_id);
CREATE INDEX idx_credibility_score ON credibility_scores(score DESC);

-- ============================================================================
-- CREDIBILITY SCORE HISTORY TABLE
-- ============================================================================
CREATE TABLE credibility_score_history (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_credibility_history_score_id ON credibility_score_history(credibility_score_id);
CREATE INDEX idx_credibility_history_timestamp ON credibility_score_history(timestamp DESC);
...
CREATE INDEX idx_logistics_transaction_id ON logistics_orders(transaction_id);
CREATE INDEX idx_logistics_provider_id ON logistics_orders(provider_id);
...
CREATE INDEX idx_storage_farmer_id ON storage_bookings(farmer_id);
CREATE INDEX idx_storage_provider_id ON storage_bookings(provider_id);
CREATE INDEX idx_storage_status ON storage_bookings(status);
...
CREATE INDEX idx_auction_listing_id ON auction_listings(listing_id);
CREATE INDEX idx_auction_farmer_id ON auction_listings(farmer_id);
CREATE INDEX idx_auction_status ON auction_listings(status);
CREATE INDEX idx_auction_end_time ON auction_listings(end_time);
...
CREATE TABLE government_schemes (
  ...
  crop_types TEXT[], -- Array of eligible crop types
  locations TEXT[], -- Array of eligible locations
  farmer_categories TEXT[], -- Array of eligible farmer categories
...
CREATE INDEX idx_scheme_apps_scheme_id ON scheme_applications(scheme_id);
CREATE INDEX idx_scheme_apps_farmer_id ON scheme_applications(farmer_id);
...
CREATE TABLE route_optimizations (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  provider_id UUID NOT NULL REFERENCES service_providers(id) ON DELETE CASCADE,
  pickup_points JSONB NOT NULL, -- Array of {farmerId, location, pickupTime}
  delivery_lat DECIMAL(10, 8) NOT NULL,
  delivery_lng DECIMAL(11, 8) NOT NULL,
  optimized_route JSONB NOT NULL, -- Array of location coordinates
...
CREATE INDEX idx_route_provider_id ON route_optimizations(provider_id);
CREATE INDEX idx_route_created_at ON route_optimizations(created_at DESC);
...
CREATE TABLE vehicle_tracking (
  ...
  speed DECIMAL(5, 2) NOT NULL, -- in km/h
  estimated_arrival TIMESTAMP NOT NULL,
  last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_vehicle_tracking_order_id ON vehicle_tracking(logistics_order_id);
CREATE INDEX idx_vehicle_tracking_vehicle_id ON vehicle_tracking(vehicle_id);
CREATE INDEX idx_vehicle_tracking_last_updated ON vehicle_tracking(last_updated DESC);
...
CREATE INDEX idx_dispute_evidence_timestamp ON dispute_evidence(timestamp DESC);
